            ...
            <div className="w-full max-w-3xl mx-auto py-8 animate-in fade-in zoom-in duration-500">
                <div id="essay-scroll-point" className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-100 mb-8">
                    <div className="text-center mb-10 border-b-2 border-primary/20 pb-6">
                        <h1 className="text-2xl font-bold text-primary">Tes Kemampuan Berpikir Kritis</h1>
                    </div>
                    ...
                    <div className="space-y-4">
                        <label className="block text-sm font-bold text-slate-600">
                            Tuliskan jawaban dan penjelasan lengkap Anda di bawah ini:
                        </label>
                        <textarea
                            ref={textareaRef}
                            value={val}
                            ...
                            className="w-full h-64 sm:h-80 p-5 text-lg border-2 border-slate-200 rounded-2xl focus:border-primary focus:ring-4 focus:ring-primary/20 outline-none resize-none transition-all placeholder:text-slate-300 font-medium"
                            ...
                            <div className="bg-slate-900 p-4 md:p-6 rounded-3xl shadow-2xl animate-in slide-in-from-top-4 duration-300">
                                <div className="flex flex-col md:flex-row gap-6">
                                    <div className="grid grid-cols-4 gap-2 flex-1">
                                        {mathSymbols.map((s, i) => (
                                            <button key={i} type="button" onClick={() => insertMathSymbol(s.sym)} className={`h-11 sm:h-14 rounded-xl text-lg sm:text-xl font-bold transition-all active:scale-95 flex items-center justify-center ${s.type === 'num' ? 'bg-slate-700 text-white hover:bg-slate-600' : 'bg-primary text-white hover:bg-primary-hover'}`}>{s.sym}</button>
                                        ))}
                                    </div>
                                    <div className="grid grid-cols-2 md:grid-cols-2 gap-2 flex-1">
                                        {advancedSymbols.map((s, i) => (
                                            <button key={i} type="button" onClick={() => insertMathSymbol((s as any).insert || s.sym)} className="h-11 sm:h-14 bg-slate-800 text-white border border-slate-700 rounded-xl hover:bg-slate-700 transition-all active:scale-95 group relative flex flex-col items-center justify-center pt-1">
                                                {s.sym === "/" ? (
                                                    <div className="flex flex-col items-center justify-center -space-y-1.5 transform scale-75 sm:scale-90 mb-1"><span className="text-xs sm:text-sm font-bold">a</span><div className="w-3 sm:w-4 h-0.5 bg-white rounded-full"></div><span className="text-xs sm:text-sm font-bold">b</span></div>
                                                ) : (
                                        ...
                                        <button type="button" onClick={() => { const newText = val.slice(0, -1); const newMap = { ...essayAnswers, [essayStep]: newText }; setEssayAnswers(newMap); autoSaveEssay(newMap); }} className="h-11 sm:h-14 bg-rose-600 text-white rounded-xl hover:bg-rose-700 transition-all active:scale-95 flex items-center justify-center font-bold col-span-2 text-xs sm:text-base">HAPUS ⌫</button>
                                    </div>
                                </div>
                            </div>
                        )}
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                    ...
                    <div className="flex gap-2 w-full sm:w-auto">
                        {!isFirst && (
                            <button onClick={handleEssayPrev} className="flex-1 sm:flex-none border-2 border-slate-200 hover:border-slate-300 text-slate-600 font-bold py-3 px-6 rounded-xl transition-all">Kembali</button>
                        )}
                        <button onClick={handleEssayNext} className={`flex-1 sm:flex-none font-bold py-3 px-8 rounded-xl transition-all shadow-md ${isLast ? 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-emerald-500/20' : 'bg-primary hover:bg-primary-hover text-white'}`}>{isLast ? "Selesaikan Tes" : "Lanjut"}</button>
                    </div>
                </div>
            </div>
        );
                    ...
                    <div className="flex justify-between items-center mb-3">
                        <div>
                            <span className="font-bold text-slate-800 block text-sm sm:text-base">
                                ...
                                {step === 3 && "Tahap 3: Kemampuan Berpikir Kritis"}
                            </span>
