...
export default function AdminDashboard() {
    ...
    const calculateScore = (angkets?: Record<number, number>) => {
        if (!angkets) return 0;
        // Exclude trap question (index 40) from scoring
        return Object.entries(angkets)
            .filter(([idx]) => Number(idx) !== 40)
            .reduce((acc, [_, curr]) => acc + curr, 0);
    };
    ...
    const analyzeRespondent = (s: Student) => {
        ...
        // Kebiasaan Berpikir Q41 (idx 40): "pilih 'Tidak Setuju' (TS)" -> jawaban benar = 2
        if (s.angkets_1 && s.angkets_1[40] !== undefined) {
            if (s.angkets_1[40] !== 2) {
                statsEnv.trap = "red";
                issuesEnv.push(`Gagal Soal Jebakan (Kebiasaan Berpikir Q41): Menjawab ${s.angkets_1[40]}, seharusnya 2 (TS).`);
            }
        }

        // Efikasi Diri Q41 (idx 40): "pilihlah 'Sangat Setuju' (SS)" -> jawaban benar = 5
        if (s.angkets_2 && s.angkets_2[40] !== undefined) {
            if (s.angkets_2[40] !== 5) {
                statsEfi.trap = "red";
                issuesEfi.push(`Gagal Soal Jebakan (Efikasi Diri Q41): Menjawab ${s.angkets_2[40]}, seharusnya 5 (SS).`);
            }
        }
        ...
        // -- Kebiasaan Berpikir (X1) --
        // Dim I: Q1 (idx 0, positif: tetap berusaha) vs Q5 (idx 4, negatif: berhenti)
        // Dim II: Q9 (idx 8, positif: berpikir sebelum jawab) vs Q14 (idx 13, negatif: terburu-buru)
        // Dim III: Q17 (idx 16, positif: memikirkan cara terbaik) vs Q22 (idx 21, negatif: jarang mengecek)
        // Dim IV: Q25 (idx 24, positif: menerima pendapat) vs Q31 (idx 30, negatif: mengabaikan ide)
        const envPairs = [[0, 4], [8, 13], [16, 21], [24, 30]];
        let logicEnvCount = 0;
        ...
        // Dim III: Q17 (idx 16, positif: memikirkan cara terbaik) vs Q22 (idx 21, negatif: jarang mengecek)
        // Dim IV: Q25 (idx 24, positif: menerima pendapat) vs Q31 (idx 30, negatif: mengabaikan ide)
        const efiPairs = [[0, 4], [8, 13], [16, 21], [24, 30]];
        let logicEfiCount = 0;
    ...
    const exportInstrumentBlueprint = () => {
        ...
        const envNegative = [4, 5, 6, 7, 12, 13, 14, 15, 20, 21, 22, 23, 28, 29, 30, 31, 36, 37, 38, 39];
        ...
        // 2. Efikasi Diri
        const efiNegative = [4, 5, 6, 7, 12, 13, 14, 15, 20, 21, 22, 23, 28, 29, 30, 31, 36, 37, 38, 39];
        ...
        XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([blueprintHeader, ...envBlueprintData]), "Skala Kebiasaan Berpikir");
                                                ...
                                                <td className="p-4 text-center">
                                                    <div className="flex flex-col items-center gap-1">
                                                        ...
                                                        {s.angkets_1 && calculateScore(s.angkets_1) > 0 && (
                                                            <button
                                                                onClick={(e) => { e.stopPropagation(); const val = getScaleValidity(calculateScore(s.angkets_1), 160); if (val) setPopover({ text: val.desc, x: e.clientX, y: e.clientY, idx: `v1-${s.id}` }); }}
                                                                className={`text-[9px] px-2 py-0.5 rounded-full border font-bold hover:scale-105 transition-transform ${getScaleValidity(calculateScore(s.angkets_1), 160)?.color}`}
                                                            >
                                                                {getScaleValidity(calculateScore(s.angkets_1), 160)?.label}
                                                            </button>
                                                        )}
                                                    </div>
                                                </td>
                                                <td className="p-4 text-center">
                                                    <div className="flex flex-col items-center gap-1">
                                                        ...
                                                        {s.angkets_2 && calculateScore(s.angkets_2) > 0 && (
                                                            <button
                                                                onClick={(e) => { e.stopPropagation(); const val = getScaleValidity(calculateScore(s.angkets_2), 160); if (val) setPopover({ text: val.desc, x: e.clientX, y: e.clientY, idx: `v2-${s.id}` }); }}
                                                                className={`text-[9px] px-2 py-0.5 rounded-full border font-bold hover:scale-105 transition-transform ${getScaleValidity(calculateScore(s.angkets_2), 160)?.color}`}
                                                            >
                                                                {getScaleValidity(calculateScore(s.angkets_2), 160)?.label}
                                                            </button>
                                                        )}
                                                    </div>
                                                </td>
